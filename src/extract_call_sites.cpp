...
void thread_function(vector<pair<long, long>> func_addr, unordered_map<long, call_site_info> &call_sites, string new_target_binary, ocolos_env* ocolos_environ){
   ...
   for (unsigned i=0; i<func_addr.size(); i++){
      string start_addr = convert_long_2_hex_string(func_addr[i].first);
      string stop_addr = convert_long_2_hex_string(func_addr[i].first + func_addr[i].second);
      ...
      while ((scn = elf_nextscn(e, scn)) != NULL) {
         ...
         if (strcmp(name, ".text") == 0) {      
            ...
            if (start_addr_ < section_addr || stop_addr_ > section_addr + shdr.sh_size) {
               fprintf(stderr, "text section starting addr: 0x%lx, stop addr: 0x%lx\n", 
                  section_addr, section_addr + shdr.sh_size);
               fprintf(stderr, "function start addr: 0x%lx, stop addr: 0x%lx\n", start_addr_, stop_addr_);
               fprintf(stderr, "function address range not in .text section\n");
               break;
            }
            ...
            for (size_t j = 0; j < count; j++) {
               ...
               if (insn[j].id == X86_INS_CALL && insn[j].detail != NULL) {
                  ...
                  if (x86->op_count > 0 && x86->operands[0].type == X86_OP_IMM) {
                     ...
                     call_info.belonged_func = func_addr[i].first;
...
int main(){
   ...
   vector<pair<long, long>> func_addr = extract_function_ranges(new_target_binary);

   int size =(int)( func_addr.size()/N);

   // split function starting addresses into different 
   // vectors for parallel computing
   vector<vector<pair<long, long>>> func_addr_array;
   for (int i=0; i<N; i++){
      if (i!=(N-1)){
         vector<pair<long, long>>::const_iterator first = func_addr.begin()+i*size;
         vector<pair<long, long>>::const_iterator last = func_addr.begin()+(i+1)*size;
         vector<pair<long, long>> new_str(first, last);
         func_addr_array.push_back(new_str);
      }
      else{
         vector<pair<long, long>>::const_iterator first = func_addr.begin()+i*size;
         vector<pair<long, long>>::const_iterator last = func_addr.end();
         vector<pair<long, long>> new_str(first, last);
         func_addr_array.push_back(new_str);
      }
   }
   ...
   for (int i=0; i<N; i++){
      thread t(thread_function, func_addr_array[i], ref(call_sites_array[i]), new_target_binary, &ocolos_environ);		
